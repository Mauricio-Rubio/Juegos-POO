...
public class DataBase {
    ...
    public boolean guardarJugadores(Estudiante[] students) {
        ...
        try {
            fichero = new FileOutputStream("src/DataBase/students.txt");
    ...
    public boolean guardarGanadores(Estudiante[] students) {
        ...
        try {
            if (contador1 == 0) {
                fichero = new FileOutputStream("src/DataBase/dia1.txt");
            }
            if (contador1 == 1) {
                fichero = new FileOutputStream("src/DataBase/dia2.txt");
            }
            if (contador1 == 2) {
                fichero = new FileOutputStream("src/DataBase/dia3.txt");
            }
            ObjectOutputStream tuberia = new ObjectOutputStream(fichero);
            tuberia.writeObject(students);
        } catch (FileNotFoundException e) {
    ...
    public Estudiante[] leerPartidas() {
        ...
        try {
            if (contador3 == 0){
            f1= new File("src/DataBase/dia1.txt");
            }else if(contador3 == 1){
                f1= new File("src/DataBase/dia2.txt");
            }else if(contador3 == 3){
                f1= new File("src/DataBase/dia3.txt");
            }
    ...
    public Estudiante[] leerEstudiantes() {
        ...
        Estudiante[] students = null;
        try {
            File f1 = new File("src/DataBase/students.txt");
            if (f1.exists() && !f1.isDirectory()) {
                ficheroEntrada = new FileInputStream("src/DataBase/students.txt");
                ObjectInputStream tuberiaEntrada = new ObjectInputStream(ficheroEntrada);
                students = (Estudiante[]) tuberiaEntrada.readObject();
            }else{
                return null;
